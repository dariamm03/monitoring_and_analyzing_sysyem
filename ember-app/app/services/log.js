...
export default Service.extend(Evented, {
  ...
  */
  ...
  _checkMessageOnSkipped(category, message) {
    ...
    category.name === messageCategory.promise.name && !this.get('storePromiseErrors') || message === 'TransitionAborted') {
  ...
  */
  _sendMessageToService(message, category, formattedMessage) {
    ...
    let appConfig = getOwner(this).factoryFor('config:environment').class;
    const token = this.get('session.data.authenticated.token') || this.get('session.data.authenticated.access_token');
